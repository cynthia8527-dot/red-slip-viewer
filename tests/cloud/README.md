...
- 測試專案已部署 `shipments` Edge Function v9，來源為 `../../supabase/functions/shipments/index.ts`，只把所有 `factory-photos` Storage 呼叫映射為 `factory-photos-test`；建構規則在 `shipments-test-source.mjs`，T025 會拒絕主專案 ID 並核對替換次數。POST 先驗證必填欄位，再呼叫單一資料庫函式建立群組與貨件；換群組的 PATCH 也由單一資料庫函式完成。照片上傳後改由此函式確認物件存在並建立資料列，資料列失敗時以服務權限刪除本次物件；相同路徑重送會回傳原關聯，前端網路層無回應時重試一次。草稿 `board/` 現在上傳前儲存同帳號分頁待辦，重新整理沿用同一路徑補關聯；物件暫未出現時保留兩分鐘，超時或關閉分頁仍可能留下孤兒圖。離線瀏覽器 smoke 已加入全程本機攔截的貨件建立成功但回應遺失後再次按送出，以及照片關聯逾時與重新整理演練；這些不算已登入測試專案整合。永久刪除會先以資料庫交易保存 Storage 路徑並刪除關聯資料，再清 Storage 及記錄結果。部署仍要求 JWT。主環境函式**未部署此修正**。
...
- 貨件照片指令 `npm run test:cloud:shipment-photos` 同樣只接受測試專案的本機安全憑證，固定網址／私有桶。以隨機貨件及單一隨機路徑驗證四項 T021/T030 已登入情境：物件不存在拒絕且沒有資料列；真實上傳、首次關聯與同路徑重送只留一列；錯位路徑被拒絕且原圖仍在；作廢後永久刪除並核對貨件、照片資料列及物件均為零。清理時只查本次隨機廠商名及精確物件路徑，透過 Edge 永久刪除並可重試三次，最後登出；清理仍失敗就回報 Failed。缺憑證／專案 ID 不符會在任何請求前 Failed，不記 skipped。**這四項目前尚未用真實登入執行，不能列 Passed**；不涵蓋故意令 Storage 清理失敗或一般員工的真實帳號測試。

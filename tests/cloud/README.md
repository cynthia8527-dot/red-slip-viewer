...
- 固定指令 `npm run test:cloud` 只對寫死的測試專案 URL 登入，執行「缺必填欄位、正常新增、POST 第二步失敗回滾、PATCH 第二步失敗回滾、正常換群組」五種情境，檢查關聯與原貨件不變，清除它自己建立的群組／貨件並全域登出。缺本機測試憑證會**失敗，不會 skipped**。它不包含 Google Drive、完整 UI、重送防重或所有資料關聯；不得把它算進離線 `npm run test` 的綠燈，也不得把主專案金鑰放進 GitHub Actions。
...
主環境正式 migration 的前置檢查與未解決項見 `MAIN_MIGRATION_REVIEW.md`；這次沒有更動主環境。測試專案匿名登入維持關閉；T026 的交易內假使用者 RLS 測試與已登入 Auth／Edge／Storage smoke 各自記錄。下一步仍需處理前端快速新增的多步寫入，補 API 重送防重與更完整的已登入測試，並審核主環境正式 migration。Google Drive 備份還原仍依要求延後。

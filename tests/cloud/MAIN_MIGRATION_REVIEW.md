...
此檔只記錄審核條件，**不是可執行 migration**。主 Supabase 專案 `icqdmzndjmxffnlciijs` 目前未套用本階段的三項限制，也沒有貨件新增／換群組、快速新增商品或可重試永久刪除函式；不得對主專案執行 `tests/cloud/Txxx.sql`、`enforce_confirmed_data_contracts.sql`、`create_shipment_atomic.sql`、`update_shipment_atomic.sql`、`create_product_with_initial_price.sql` 或 `delete_shipment_with_cleanup_job.sql` 這些測試專案專用 SQL。

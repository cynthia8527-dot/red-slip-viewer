...
此檔只記錄審核條件，**不是可執行 migration**。主 Supabase 專案 `icqdmzndjmxffnlciijs` 目前未套用本階段的三項限制，也沒有新增貨件交易函式；不得對主專案執行 `tests/cloud/Txxx.sql`、`enforce_confirmed_data_contracts.sql` 或 `create_shipment_atomic.sql` 這些測試專案專用 SQL。
...
- 若之後決定把新增貨件原子性修正部署到主環境，須先產生正式且不含測試專案防呆的函式 migration，核對 `security invoker` 與僅 `service_role` 可執行，再部署依賴它的 Edge Function；不可先部署新 Edge Function，否則新增貨件會因找不到 RPC 而失敗。

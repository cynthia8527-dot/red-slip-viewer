...
- `price-history-rehearsal.mjs` 在測試專案內建立舊式暫存價格表，插入 70 元，再使用歷史價格 migration 的原表 DDL（只映射目標到暫存表）兩次套用並回滾；確認舊列、日期限制、非負價格限制及索引。函式、授權與所有正式候選 migration 的完整重播仍未覆蓋。不得把此局部演練當成可直接部署的正式 migration。

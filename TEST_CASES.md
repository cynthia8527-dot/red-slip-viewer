...
| T028 | 貨件建立成功但回應逾時後重送不重複 | 相同請求 UUID 與內容；另以同 UUID 改內容 | 連續送出兩次，模擬第一次回應遺失 | 相同內容回傳同一貨件且只留一筆；同 key 改內容回 409 | 是 | Edge 與網頁離線測試通過；`tests/cloud/T028.sql` 在測試專案連跑兩次通過，Edge v7 已部署；已登入 API 情境已加入固定指令但本環境無安全憑證，尚未執行 |
...
| T021 | 真實圖片上傳、關聯、清除 | 獨立雲端測試專案的 Storage bucket | 寫入、修改錯位貨件照片路徑；上傳／清除 | 錯位被拒絕且不留下孤兒 | 是，部分通過 | SQL 路徑限制與先前真實上傳／永久刪除通過；草稿分支另把照片關聯交給 Edge Function，離線驗證資料列失敗會清除本次物件、相同路徑重送不誤刪，測試 Edge v7 已部署。失敗清理的已登入 API 情境仍待安全憑證環境執行；主環境未部署 |
...
4. T021 原本在獨立雲端測試庫失敗。測試專案已加照片路徑必須以所屬貨件 ID 開頭的資料庫限制並通過；**主環境未套用**。草稿分支新增 Edge Function 照片關聯端點：資料列失敗會以服務權限清除剛上傳物件，相同路徑重送則回傳原關聯。離線測試通過、測試 Edge v7 已部署，但失敗清理仍待已登入 API 實測；Storage 與資料庫不是同一交易，不能宣稱完全無孤兒風險。
...
7. 草稿分支已加入貨件 POST 的 `Idempotency-Key` 設計與測試專案專用候選 SQL：同一 key 先取得交易級 advisory lock，再查既有結果；同內容回傳原貨件，不同內容拒絕。網頁把尚未確知結果的 key 保存在 `sessionStorage`。候選 SQL 已只在測試專案安裝，T028 SQL 連跑兩次通過且沒有殘留貨件／群組，測試 Edge Function v7 已部署；登入 API 重送因本環境沒有專用帳號密碼尚未執行。主環境未部署。

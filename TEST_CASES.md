...
| T032 | 商品照片關聯成功但回應逾時後可安全重送 | 私有測試桶物件、商品及同一路徑重送；另模擬關聯失敗與舊圖清理失敗 | 上傳後呼叫獨立 Edge Function；同路徑重送，換圖時重試舊圖清理 | 同一路徑只關聯一次且不誤刪；關聯失敗清除本次新物件；舊圖清理可重試；一般員工被拒絕 | 是 | `tests/product-photo-edge.test.mjs` 五種離線情境通過；測試專案 `product-photos` v2 已部署且 `verify_jwt=true`、回讀確認只使用 `factory-photos-test`。本環境無安全登入憑證，尚未執行真實上傳／登入 API；主環境未部署 |
...
10. `calculator/` 商品照片改由獨立 `product-photos` Edge Function 關聯。函式以真實使用者 JWT 查 profile，只允許啟用中的管理員；用服務端權限確認私有桶物件後更新商品。同一路徑重送回傳既有結果；關聯失敗清除新物件；換圖後舊物件清理失敗可用相同請求重試。測試專案 v2 已部署並回讀確認桶名，但真實登入上傳尚未執行。上傳後在第一次關聯前立刻關閉瀏覽器，仍可能留下孤兒檔，後續需 pending recovery 或定期掃描。

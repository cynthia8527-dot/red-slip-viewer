...
| T032 | 商品照片關聯成功但回應逾時後可安全重送 | 私有測試桶物件、商品及同一路徑重送；另模擬關聯失敗與舊圖清理失敗 | 上傳前保存待辦；重開同一分頁沿用路徑呼叫獨立 Edge Function；換圖時重試舊圖清理 | 同一路徑只關聯一次且不誤刪；關聯失敗清除本次新物件；舊圖清理可重試；一般員工被拒絕 | 是 | `tests/product-photo-edge.test.mjs` 五種離線情境及 `tests/product-photo-pending.test.mjs` 五種離線情境通過；新待辦遇暫時找不到物件會保留兩分鐘，以免上傳晚於重新開頁時的檢查。測試專案 `product-photos` v2 已部署且 `verify_jwt=true`、回讀確認只使用 `factory-photos-test`。本環境無安全登入憑證，尚未執行真實上傳／登入 API；主環境未部署 |

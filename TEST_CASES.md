...
- `tests/fixtures/schema.sql`、`legacy.sql`、`migration.sql` 仍是**合成契約夾具**。另已把主專案記錄的 17 筆 migration SQL 與 `shipments` Edge Function 原始碼取回存於 `tests/reference/`，只作唯讀參考，不自動部署。獨立測試專案已重播與業務表有關的歷史 SQL，排除備份排程及過時的公開讀取政策；缺失的叫車表及索引以 `tests/cloud/` 的現況重建 SQL 補足。沒有複製主環境資料。
...
| T010 | 相同內容不得意外重複建立 | 合成資料、獨立測試庫 | 重複建立相同廠商與叫車地點 | 第二筆拒絕 | 是 | `tests/data.test.mjs`＋`tests/cloud/T010.sql`；**雲端失敗**，現況沒有相應唯一鍵；精確判重規則待確認 |
...
| T021 | 真實圖片上傳、關聯、清除 | 獨立雲端測試專案的 Storage bucket | 寫入錯位貨件照片路徑；後續上傳／清除 | 錯位被拒絕且不留下孤兒 | 是，部分失敗 | `tests/cloud/T021.sql`；**資料庫接受錯位路徑**；Storage 實際上傳／清除尚未跑 |
...
3. T010 在獨立雲端測試庫實際失敗：兩筆除了 ID 外相同的廠商資料、叫車地點資料都被接受。先請產品端確認「重複」的精確判定欄位，再決定唯一鍵／冪等策略；不能偷改測試預期。
4. T021 在獨立雲端測試庫實際失敗：`shipment_photos.storage_path` 可以指向另一筆貨件的資料夾。照片資料列與 Storage 實體檔案亦尚未整合驗證。

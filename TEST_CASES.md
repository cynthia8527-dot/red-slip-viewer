...
本文件把「不希望改壞的核心規則」與「尚未定案的功能」分開。`npm run test` 是**離線**完整指令，通過不等於雲端整合測試通過。`npm run test:cloud` 執行獨立測試專案的已登入 API 情境；其餘雲端 SQL 情境放在 `tests/cloud/`，透過 Supabase 連接工具逐一執行，不計入離線 passed。這個缺口不能用 skipped 或綠燈掩蓋。任何測試若 skipped、失敗或無測試，都不得宣稱整套通過。情境編號為 T001–T032；T020、T022 仍延後或待確認。T019 另有兩項歷史 migration 文字盤點離線測試，僅指出 baseline 缺漏，不能當成正式 migration 重播通過。
...
| T030 | 永久刪除不留下壞掉的照片關聯，清理失敗可重試 | 已作廢貨件、照片與私有清理工作；另故意讓關聯刪除或 Storage 清理失敗 | 先以單一資料庫交易保存圖片路徑並刪貨件，再清 Storage；重送同一刪除 | 關聯刪除失敗時貨件／照片／工作整筆回滾；Storage 失敗時貨件已刪且工作保留，重送完成清理，不重複刪資料 | 是 | 離線 Edge 三情境通過；`tests/cloud/T030.sql` 在測試專案連跑兩次通過且合成貨件／照片／工作均為 0。測試 Edge v9 已部署；`npm run test:cloud:shipment-photos` 已備妥正常登入刪除與照片清理案例，但尚未執行，也尚未以登入 API 故意觸發 Storage 清理失敗；主環境未套用 |
...
|---|---|---|---|---|---|---|
| T017 | 真實 API 防重與交易性 | 獨立測試庫及安全的測試身分 | 重送建立請求並注入中途錯誤 | 不重複、無半套資料 | 是，尚未完全驗證 | 已登入 `shipments` API 的 POST 與換群組 PATCH 第二步失敗均在測試專案驗證回滾；重送防重的資料庫 SQL 已通過且測試 Edge 已部署。離線瀏覽器另模擬伺服器已建立、首次回應遺失後由使用者再次按送出，驗證同 key／同內容且畫面只有一筆；這仍不算測試專案登入 API 重送通過，需在安全憑證環境執行；見 T028 與 `tests/cloud/README.md` |
...
| T021 | 真實圖片上傳、關聯、清除 | 獨立雲端測試專案的 Storage bucket | 寫入、修改錯位貨件照片路徑；上傳／清除 | 錯位被拒絕且不留下孤兒 | 是，部分通過 | SQL 路徑限制與先前真實上傳／永久刪除通過；草稿分支把照片關聯交給 Edge Function，離線驗證物件存在、資料列失敗清除、同路徑安全重送及網路無回應重試。`tests/shipment-photo-pending.test.mjs` 五項離線案例涵蓋上傳前保存、同帳號同分頁重開補關聯、逾時與近期物件尚未出現；瀏覽器 smoke 另以本機攔截的登入／Edge 回應模擬首次逾時後重新整理，檢查同路徑只補關聯一次。`npm run test:cloud:shipment-photos` 四個已登入案例已備妥但尚未實際執行，不能列 Passed；關閉分頁或超過兩分鐘仍可能遺留物件，預期不變。測試 Edge v9 已部署；主環境未部署 |

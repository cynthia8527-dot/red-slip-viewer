...
| T019 | 真實 migration 舊資料相容 | 歷史 SQL 與合成舊版樣本 | 獨立測試庫先放舊價格再套價格歷史 migration；另在已更新結構下用舊欄位寫入／修改合成商品、價格及貨件 | 舊值保留；舊式讀寫仍可用 | 是，部分通過 | `tests/reference/main-migrations/` 的真實分階段驗證曾通過一次，尚無一鍵重建重跑；`tests/cloud/T019_legacy_writes.sql` 在測試專案連跑兩次通過且測後 0 筆，只驗證更新後舊式讀寫，不等於重新套 migration |

...
| T031 | 快速新增商品成功但回應逾時後重送不重複 | 相同 request UUID 與商品／初始價格；另以同 UUID 改商品名稱 | 連續與兩連線並發呼叫新 RPC，模擬第一次回應遺失 | 相同內容回傳同一商品且只留一筆商品／價格；改內容回 409；舊 RPC 仍可用 | 是 | 離線網頁防呆通過；`tests/cloud/T031.sql` 在測試專案連跑兩次通過，兩連線並發得到一新建、一 replay，測後商品／價格／測試帳號均為 0。登入 API 案例已加入但本環境無安全憑證，尚未執行；主環境未套用 |
...
9. 快速新增商品候選新增可空 request UUID／fingerprint 欄位與獨立 RPC，不覆蓋舊函式。網頁把不確定結果的 key 保存在 `sessionStorage`；相同 key／內容回傳原商品，改內容以 HTTP 409 拒絕。T031 SQL、舊呼叫及並發已通過；登入固定指令已加入兩項但尚未執行。

...
- 建立測試專案後已用該專案 ID 執行一次唯讀 SQL `select 1`，並確認測試圖片桶存在。這只是雲端連線／隔離探測，**不是** T017–T021 的真實資料整合測試，也不計入 `npm run test` 的通過數。
...
|---|---|---|---|---|---|---|
| T017 | 真實資料庫防重與交易性 | 獨立雲端測試專案、版本化 schema／Functions | 重送建立請求並注入中途錯誤 | 不重複、無半套資料 | 是，整合驗證待補 | 尚無；不得指向主環境 |
| T018 | 真實價格替換與出貨快照 | 獨立雲端測試專案、`replace_vendor_price`／`shipments` 原始碼 | 換價、出貨、再換價 | 歷史不覆蓋且快照不改 | 是，整合驗證待補 | 尚無；後端原始碼未入倉 |
| T019 | 真實 migration 舊資料相容 | 真實 baseline schema 與合成舊版樣本 | 僅對獨立雲端測試庫跑 migration | 舊資料可讀、關聯完整 | 是，整合驗證待補 | 尚無；後端 migration 未入倉 |
| T020 | Google Drive 備份可還原 | 獨立測試雲端位置與測試備份 | 備份、刪除測試副本、還原 | 內容及 ID／關聯一致 | 是，延後 | 尚無；本階段不碰雲端備份 |
| T021 | 真實圖片上傳、關聯、清除 | 獨立雲端測試專案的 Storage bucket | 上傳／刪除測試圖片 | 路徑與資料關聯正確，不留下孤兒 | 是，整合驗證待補 | 尚無；目前僅測試 fixture／前端設定 |

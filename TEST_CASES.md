...
| T032 | 商品照片關聯成功但回應逾時後可安全重送 | 私有測試桶物件、商品及同一路徑重送；另模擬關聯失敗與舊圖清理失敗 | 上傳前保存待辦；重開同一分頁沿用路徑呼叫獨立 Edge Function；換圖時重試舊圖清理 | 同一路徑只關聯一次且不誤刪；關聯失敗清除本次新物件；舊圖清理可重試；一般員工被拒絕 | 是 | `tests/product-photo-edge.test.mjs` 五種離線情境及 `tests/product-photo-pending.test.mjs` 四種離線情境通過；測試專案 `product-photos` v2 已部署且 `verify_jwt=true`、回讀確認只使用 `factory-photos-test`。本環境無安全登入憑證，尚未執行真實上傳／登入 API；主環境未部署 |
...
| T021 | 真實圖片上傳、關聯、清除 | 獨立雲端測試專案的 Storage bucket | 寫入、修改錯位貨件照片路徑；上傳／清除 | 錯位被拒絕且不留下孤兒 | 是，部分通過 | SQL 路徑限制與先前真實上傳／永久刪除通過；草稿分支另把照片關聯交給 Edge Function，離線驗證物件存在、資料列失敗清除、同路徑安全重送及網路無回應重試，測試 Edge v9 已部署。新流程的已登入 API 情境仍待安全憑證環境執行；主環境未部署 |
...
4. T021 原本在獨立雲端測試庫失敗。測試專案已加照片路徑必須以所屬貨件 ID 開頭的資料庫限制並通過；**主環境未套用**。草稿分支新增 Edge Function 照片關聯端點：先確認 Storage 物件存在，資料列失敗會以服務權限清除本次物件，相同路徑重送則回傳原關聯；前端遇網路層無回應時以同一路徑重試一次。離線測試通過、測試 Edge v9 已部署，但新流程仍待已登入 API 實測；Storage 與資料庫不是同一交易，瀏覽器在重試前關閉仍可能留下孤兒檔。
...
10. `calculator/` 商品照片改由獨立 `product-photos` Edge Function 關聯。函式以真實使用者 JWT 查 profile，只允許啟用中的管理員；用服務端權限確認私有桶物件後更新商品。同一路徑重送回傳既有結果；關聯失敗清除新物件；換圖後舊物件清理失敗可用相同請求重試。網頁在上傳前於分頁內保存待辦，同帳號重新開頁會接續，確定物件不存在才移除待辦。測試專案 v2 已部署並回讀確認桶名，但真實登入上傳尚未執行。關閉整個分頁／清除瀏覽器資料時 sessionStorage 可能消失，仍需定期孤兒掃描或持久工作。

...
本文件把「不希望改壞的核心規則」與「尚未定案的功能」分開。`npm run test` 是**離線**完整指令，通過不等於雲端整合測試通過。`npm run test:cloud` 執行獨立測試專案的已登入 API 情境；其餘雲端 SQL 情境放在 `tests/cloud/`，透過 Supabase 連接工具逐一執行，不計入離線 passed。這個缺口不能用 skipped 或綠燈掩蓋。任何測試若 skipped、失敗或無測試，都不得宣稱整套通過。情境編號為 T001–T030；T020、T022 仍延後或待確認。
...
| T028 | 貨件建立成功但回應逾時後重送不重複 | 相同請求 UUID 與內容；另以同 UUID 改內容 | 連續送出兩次，模擬第一次回應遺失 | 相同內容回傳同一貨件且只留一筆；同 key 改內容回 409 | 是 | Edge 與網頁離線測試通過；`tests/cloud/T028.sql` 在測試專案連跑兩次通過，Edge v8 已部署；已登入 API 情境已加入固定指令但本環境無安全憑證，尚未執行 |
...
| T030 | 永久刪除不留下壞掉的照片關聯，清理失敗可重試 | 已作廢貨件、照片與私有清理工作；另故意讓關聯刪除或 Storage 清理失敗 | 先以單一資料庫交易保存圖片路徑並刪貨件，再清 Storage；重送同一刪除 | 關聯刪除失敗時貨件／照片／工作整筆回滾；Storage 失敗時貨件已刪且工作保留，重送完成清理，不重複刪資料 | 是 | 離線 Edge 三情境通過；`tests/cloud/T030.sql` 在測試專案連跑兩次通過且合成貨件／照片／工作均為 0。候選 RPC 已只安裝在測試專案，但新版 Edge 尚未部署及登入實測；主環境未套用 |
...
| T021 | 真實圖片上傳、關聯、清除 | 獨立雲端測試專案的 Storage bucket | 寫入、修改錯位貨件照片路徑；上傳／清除 | 錯位被拒絕且不留下孤兒 | 是，部分通過 | SQL 路徑限制與先前真實上傳／永久刪除通過；草稿分支另把照片關聯交給 Edge Function，離線驗證物件存在、資料列失敗清除、同路徑安全重送及網路無回應重試，測試 Edge v8 已部署。新流程的已登入 API 情境仍待安全憑證環境執行；主環境未部署 |
...
4. T021 原本在獨立雲端測試庫失敗。測試專案已加照片路徑必須以所屬貨件 ID 開頭的資料庫限制並通過；**主環境未套用**。草稿分支新增 Edge Function 照片關聯端點：先確認 Storage 物件存在，資料列失敗會以服務權限清除本次物件，相同路徑重送則回傳原關聯；前端遇網路層無回應時以同一路徑重試一次。離線測試通過、測試 Edge v8 已部署，但新流程仍待已登入 API 實測；Storage 與資料庫不是同一交易，瀏覽器在重試前關閉仍可能留下孤兒檔。
...
7. 草稿分支已加入貨件 POST 的 `Idempotency-Key` 設計與測試專案專用候選 SQL：同一 key 先取得交易級 advisory lock，再查既有結果；同內容回傳原貨件，不同內容拒絕。網頁把尚未確知結果的 key 保存在 `sessionStorage`。候選 SQL 已只在測試專案安裝，T028 SQL 連跑兩次通過且沒有殘留貨件／群組，測試 Edge Function v8 已部署；登入 API 重送因本環境沒有專用帳號密碼尚未執行。主環境未部署。
8. 永久刪除候選流程不再先刪 Storage：`delete_shipment_with_cleanup_job` 在同一資料庫交易保存路徑並刪除貨件／照片，Edge 之後清 Storage 並記錄成功或錯誤。相同貨件 ID 重送會取得持久工作；Storage 失敗回 202 並可重試。T030 SQL 與離線 Edge 測試已通過，但測試 Edge 尚未部署新版、登入整合未執行，不能先稱完整通過。
